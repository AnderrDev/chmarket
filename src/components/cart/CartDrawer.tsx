      ...
      <aside className={`absolute right-0 top-0 h-full w-full max-w-md bg-ch-dark-gray shadow-2xl transform transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'} border-l border-ch-gray/20`}>
        ...
        <div className="p-6 space-y-4 max-h-[calc(100vh-200px)] overflow-y-auto">
          {items.length === 0 ? (
            ...
            <div key={keyOf(item)} className="bg-ch-medium-gray rounded-lg p-4 border border-ch-gray/30">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <button onClick={() => setQty(keyOf(item), item.quantity - 1)} className="p-1 hover:bg-ch-light-gray rounded"><Minus className="w-4 h-4 text-ch-gray"/></button>
                    <span className="px-3 py-1 bg-ch-black rounded text-sm text-white border border-ch-gray/30">{item.quantity}</span>
                    <button onClick={() => setQty(keyOf(item), item.quantity + 1)} className="p-1 hover:bg-ch-light-gray rounded"><Plus className="w-4 h-4 text-ch-gray"/></button>
                    <button onClick={() => remove(keyOf(item))} className="ml-2 text-ch-primary hover:underline">Remove</button>
                  </div>
                </div>
